/* Purpose:	Example for implementing timed_mutex_lock for locking resources 
...
void * read_thread(void *p)
{
	struct timespec mytime;
        ...
        mytime.tv_sec = time(NULL)+5;/* from current time it will wait for three seconds*/
        mytime.tv_nsec = 0;
	...
	if(pthread_mutex_timedlock(&mutex,&mytime)==0)
	{
		...
		printf ("\t Mutex not avalible for Read thread in %d seconds ...",time(NULL) - currTime);
