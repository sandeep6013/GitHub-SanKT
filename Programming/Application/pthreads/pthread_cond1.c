/* Purpose: Implementing conditional variables for signaling b/n threads(broadcast) 
 * Author : Sandeep Kumar M
 */

/* Use pthread_cond_broadcast(THR) to unblock all threads that are blocked on the condition variable pointed to by cv, 
 * specified by pthread_cond_wait(). When no threads are blocked on the condition variable, pthread_cond_broadcast() has no effect.
 * pthread_cond_broadcast() returns zero after completing successfully. Any other returned value indicates that an error occurred. 
 * Since pthread_cond_broadcast() causes all threads blocked on the condition to contend again for the mutex lock, use it with care
 */
